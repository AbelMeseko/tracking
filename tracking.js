 ...
 * Handles both mm/dd/yyyy (MAIN tab) and dd/mm/yyyy (vehicle tabs)
 * @param {string} dateStr - The date string to convert
 * @param {string} sourceFormat - 'main' for mm/dd/yyyy or 'vehicle' for dd/mm/yyyy
 * @returns {string} Date in YYYY-MM-DD format
 */
function normalizeDate(dateStr, sourceFormat = 'auto') {
    if (!dateStr) return '';
    ...
    try {
        ...
        // Clean up parts (remove any non-numeric characters)
        parts = parts.map(p => p.replace(/[^0-9]/g, ''));
        
        if (sourceFormat === 'main' || sourceFormat === 'auto') {
            // Try to determine format
            if (sourceFormat === 'main') {
                // MAIN tab uses mm/dd/yyyy
                month = parts[0].padStart(2, '0');
                day = parts[1].padStart(2, '0');
                year = parts[2];
            } else if (sourceFormat === 'vehicle') {
                ...
                if (parseInt(parts[0]) > 12) {
                    ...
                    // Both parts <= 12, assume mm/dd/yyyy (MAIN format as default)
                    month = parts[0].padStart(2, '0');
                    day = parts[1].padStart(2, '0');
                    year = parts[2];
                }
            ...
            // Default to mm/dd/yyyy (MAIN format)
            month = parts[0].padStart(2, '0');
            day = parts[1].padStart(2, '0');
            year = parts[2];
        }
        ...
        console.warn('Date normalization error:', e, 'for date:', dateStr);
        return '';
    }
}

/**
 * Convert date from vehicle format (dd/mm/yyyy) to MAIN format (mm/dd/yyyy)
 * @param {string} dateStr - Date in dd/mm/yyyy format
 * @returns {string} Date in mm/dd/yyyy format
 */
function convertVehicleDateToMainFormat(dateStr) {
    if (!dateStr) return '';
    
    try {
        // First normalize to YYYY-MM-DD assuming vehicle format (dd/mm/yyyy)
        const normalized = normalizeDate(dateStr, 'vehicle');
 ...
 * @param {string} targetFormat - 'main' for mm/dd/yyyy or 'vehicle' for dd/mm/yyyy
 * @returns {string} Formatted date
 */
function formatDateForDisplay(dateStr, targetFormat = 'main') {
    if (!dateStr) return '';
    
    try {
        const [year, month, day] = dateStr.split('-');
        
        if (targetFormat === 'main') {
            return `${month}/${day}/${year}`;
        } else {
...
function processVehicleRecords() {
    ...
    VEHICLES.forEach(vehicle => {
        ...
        vehicleData.data.forEach(row => {
            // Use vehicle format (dd/mm/yyyy) for normalization
            const date = normalizeDate(row[dateIdx] || '', 'vehicle');
            if (!date) return;
            ...
            vehicleRecords[vehicle].push({
                ...
                distance: distance,
                vehicle: vehicle,
                originalDate: row[dateIdx] || '' // Keep original for display
            });
        });
...
function processDailyTotals() {
    ...
    // Process MAIN data (uses mm/dd/yyyy format)
        ...
        if (regIdx !== -1 && dateIdx !== -1 && startIdx !== -1 && endIdx !== -1) {
            mainData.data.forEach(row => {
                ...
                if (vehicle && VEHICLES.includes(vehicle)) {
                    // Use main format (mm/dd/yyyy) for normalization
                    const date = normalizeDate(row[dateIdx] || '', 'main');
                    if (!date) return;
    ...
    // Process Vehicle tabs (uses dd/mm/yyyy format, converted to YYYY-MM-DD)
    VEHICLES.forEach(vehicle => {
        (vehicleRecords[vehicle] || []).forEach(record => {
            if (record.distance > 0.10) {
                const date = record.date; // Already normalized to YYYY-MM-DD
...
function filterVehicleRows(rows) {
    ...
    if ((vehicleTabFilters.startDate || vehicleTabFilters.endDate) && dateIdx !== -1) {
        filteredRows = filteredRows.filter(row => {
            // Use vehicle format (dd/mm/yyyy) for filtering
            const rowDate = normalizeDate(row[dateIdx] || '', 'vehicle');
...
function filterMainRows(rows) {
    ...
    return rows.filter(row => {
        ...
        if ((mainFilters.startDate || mainFilters.endDate) && dateIdx !== -1) {
            // Use main format (mm/dd/yyyy) for filtering
...
function displayTabData(tabName) {
    ...
    const endIdx = findColumnIndex(headers, 'end');
    const distIdx = findColumnIndex(headers, 'dist');
    const dateIdx = findColumnIndex(headers, tabName === 'MAIN' ? 'date' : 'stop time');
    ...
    rowsToDisplay.forEach(row => {
        ...
        headers.forEach((header, colIndex) => {
            ...
            // Format dates for display in MAIN format when viewing vehicle tabs
            if (tabName !== 'MAIN' && colIndex === dateIdx && row[colIndex]) {
                // Convert from vehicle format (dd/mm/yyyy) to MAIN format (mm/dd/yyyy) for display
                cell.textContent = convertVehicleDateToMainFormat(row[colIndex]);
            } else {
...
function updateComparisonTable(vehicles, dateRange) {
    ...
    dateRange.forEach((date, index) => {
        ...
        // Format date in MAIN format (mm/dd/yyyy) for display
        const displayDate = formatDateForDisplay(date, 'main');
...
function exportTabData() {
    ...
    const headers = [...(tabData.rawHeaders || [])];
    const dateIdx = findColumnIndex(headers, currentView === 'MAIN' ? 'date' : 'stop time');
    ...
    dataToExport.forEach(row => {
        ...
        row.forEach((cell, idx) => {
            // Convert vehicle tab dates to MAIN format for export
            if (currentView !== 'MAIN' && idx === dateIdx && cell) {
                html += '<td>' + convertVehicleDateToMainFormat(cell) + '</td>';
            } else {
                html += '<td>' + cell + '</td>';
            }
        });
...
function exportComparisonData() {
    ...
    dateRange.forEach(date => {
        html += '<tr>';
        // Format date in MAIN format (mm/dd/yyyy)
        const displayDate = formatDateForDisplay(date, 'main');
